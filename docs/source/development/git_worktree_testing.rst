...
- ``refactoring_principles.rst`` - simplification rules to apply before and after branch tests
- ``respecting_codebase_architecture.rst`` - fail-loud review guidance while checking a branch
- ``agent_refactoring_crash_course.rst`` - the agent-facing workflow for larger architectural passes
