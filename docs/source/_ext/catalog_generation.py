...
def generate_api_reference_pages(source_dir: Path) -> None:
    ...
    _write_if_changed(generated_dir / 'pattern_catalog.rst', _render_pattern_catalog(sorted(PATTERN_SPECS.values(), key=lambda item: item.pattern_id.value)))
    _write_if_changed(generated_dir / 'detector_catalog.rst', _render_detector_catalog(detector_types))
...
def _render_pattern_catalog(patterns: list[PatternSpec]) -> str:
    lines = ['.. This file is generated from nominal_refactor_advisor.patterns.PATTERN_SPECS.', '.. Do not edit manually.', '', 'This catalog is generated from ``nominal_refactor_advisor.patterns.PATTERN_SPECS``.', 'The code metadata remains the authoritative source; this page is only a rendered view.', '', 'Summary', '-------', '', '.. list-table::', '   :header-rows: 1', '', '   * - ID', '     - Name', '     - Priority', '     - Dependencies', '     - Plan Builder', '     - Action Builder']
    for pattern in patterns:
        ...
        lines.extend([f'   * - ``{pattern.pattern_id.value}``', f'     - {pattern.name}', f'     - ``{pattern.priority}``', f'     - {dependencies}', f'     - {plan_builder}', f'     - {action_builder}'])
    lines.extend(["", "Patterns", "--------", ""])
    for pattern in patterns:
        title = f"Pattern {pattern.pattern_id.value}: {pattern.name}"
        lines.extend([title, '^' * len(title), '', f':Prescription: {pattern.prescription}', f':Canonical shape: {pattern.canonical_shape}', f':Priority: ``{pattern.priority}``', f":Dependencies: {_pattern_id_list(pattern.dependencies) or 'None'}", f":Synergy: {_pattern_id_list(pattern.synergy_with) or 'None'}", f":Witness capabilities: {_capability_list(pattern.witness_capabilities) or 'None'}", f':Plan builder: ``{pattern.plan_step_builder_id.value}``' if pattern.plan_step_builder_id is not None else ':Plan builder: None', f':Action builder: ``{pattern.action_builder_id.value}``' if pattern.action_builder_id is not None else ':Action builder: None', '', 'First moves:', ''])
        ...
        if pattern.example_skeletons:
            ...
            for skeleton in pattern.example_skeletons:
                ...
                lines.extend((f'   {line}' if line else '' for line in skeleton.splitlines()))
                lines.append("")
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"
...
def _render_detector_catalog(detector_types: tuple[type[IssueDetector], ...]) -> str:
    lines = ['.. This file is generated from nominal_refactor_advisor.detectors.IssueDetector.', '.. Do not edit manually.', '', 'This catalog is generated from the registered detector family rooted at', '``nominal_refactor_advisor.detectors.IssueDetector``. The registry order is the', 'authoritative source for what the tool ships.', '', 'Summary', '-------', '', f'- Total detectors: ``{len(detector_types)}``', '', '.. list-table::', '   :header-rows: 1', '', '   * - Detector ID', '     - Pattern', '     - Finding title', '     - Class', '     - Base', '     - Genericity', '     - Priority']
    for detector_type in detector_types:
        finding_spec = _finding_spec(detector_type)
        lines.extend([f'   * - ``{detector_type.detector_id}``', f'     - ``{finding_spec.pattern_id.value}``' if finding_spec is not None else '     - Unknown', f'     - {finding_spec.title}' if finding_spec is not None else '     - Unknown', f'     - ``{detector_type.__name__}``', f'     - ``{_detector_base_name(detector_type)}``', f'     - ``{detector_type.genericity}``', f'     - ``{detector_type.detector_priority}``'])
    lines.extend(["", "Detectors", "---------", ""])
    for detector_type in detector_types:
        title = detector_type.__name__
        finding_spec = _finding_spec(detector_type)
        lines.extend([title, '^' * len(title), '', f':Detector ID: ``{detector_type.detector_id}``', f':Pattern: ``{finding_spec.pattern_id.value}``' if finding_spec is not None else ':Pattern: Unknown', f':Base: ``{_detector_base_name(detector_type)}``', f':Genericity: ``{detector_type.genericity}``', f':Priority: ``{detector_type.detector_priority}``', f':Reference: :doc:`detector_reference/{detector_type.detector_id}`', f':Summary: {_detector_summary(detector_type)}', ''])
    return "\n".join(lines).rstrip() + "\n"
