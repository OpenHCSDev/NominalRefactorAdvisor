...
class GenericPatternActionBuilder(PatternActionBuilder):
    def build(
        ...
        template = ActionTemplate(kind='apply_pattern', description=f'Apply Pattern {pattern_id.value}: {PATTERN_SPECS[pattern_id].prescription}', confidence=MEDIUM_CONFIDENCE)
...
def _estimate_outcome(
    ...
    if loci_before == 0:
        loci_before = len({(item.file_path, item.line) for finding in findings for item in finding.evidence})
...
def _build_plan_steps(
    ...
    steps.append(f'Delete superseded partial views in `{subsystem}` and route call sites through the new authorities.')
    return tuple(steps)
...
def _registry_name_from_findings(findings: tuple[RefactorFinding, ...]) -> str:
    registry_name = _first_metric_value(findings, lambda metrics: metrics.plan_registry_name, 'Registry')
...
def _statement_count_from_findings(findings: tuple[RefactorFinding, ...]) -> int:
    return int(_first_metric_value(findings, lambda metrics: metrics.plan_statement_count, 0))
...
def _unique_capabilities(findings: tuple[RefactorFinding, ...]) -> list[CapabilityTag]:
    capabilities = sorted({tag for finding in findings for tag in finding.capability_tags})
    return capabilities
