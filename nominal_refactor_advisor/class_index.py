...
def _resolve_relative_module(
    ...
    *,
    ...
    if level > 1:
        if level - 1 > len(package_parts): return None
...
def _module_import_aliases(parsed_module: ParsedModule) -> dict[str, str]:
    ...
    for statement in parsed_module.module.body:
        if isinstance(statement, ast.Import):
            ...
            if resolved_module is None: continue
            for alias in statement.names:
...
def _resolve_base_symbol(
    ...
    *,
    ...
    if alias_target is not None:
        ...
        if candidate in known_symbols: return candidate
...
def _build_class_family_index_cached(
    ...
    for _, qualname, _, symbol in class_records: symbols_by_simple_name_multimap[qualname.rsplit('.', 1)[-1]].append(symbol)
    ...
    for symbol in sorted(classes_by_symbol):
        ...
        if descendants: descendants_by_symbol[symbol] = tuple(descendants)
