...
class CapabilityTag(LabeledStrEnum):
    ...
    AUTHORITATIVE_DISPATCH = (
        "authoritative_dispatch",
        "authoritative closed-family dispatch",
        "which declared rule family owns dispatch",
    )
    AUTHORITATIVE_MAPPING = (
        "authoritative_mapping",
        "authoritative mapping ownership",
        "which mapping is the single writable source",
    )
    BIDIRECTIONAL_NORMALIZATION = (
        "bidirectional_normalization",
        "bidirectional normalization",
        "which companion type is the forward or reverse authority",
    )
    CAPABILITY_MARKER_IDENTITY = (
        "capability_marker_identity",
        "exact capability-marker identity",
        "which exact capability marker is present",
    )
    CLASS_LEVEL_REGISTRATION = (
        "class_level_registration",
        "class-level registration",
        "which classes belong in the registry",
    )
    CLOSED_FAMILY_DISPATCH = (
        "closed_family_dispatch",
        "closed-family dispatch",
        "which closed variant case applies",
    )
    DUAL_AXIS_RESOLUTION = (
        "dual_axis_resolution",
        "dual-axis precedence resolution",
        "which scope x type pair should win precedence",
    )
    ENUMERATION = (
        "enumeration",
        "exhaustive family enumeration",
        "which variants belong to the family",
    )
    EXACT_LOOKUP = (
        "exact_lookup",
        "exact reverse lookup",
        "which reverse companion should be recovered",
    )
    FAIL_LOUD_CONTRACTS = (
        "fail_loud_contracts",
        "fail-loud nominal contracts",
        "which role family a value actually belongs to",
    )
    GENERATED_INTERFACE_IDENTITY = (
        "generated_interface_identity",
        "runtime-generated interface identity",
        "which generated interface identity is being claimed",
    )
    MRO_ORDERING = (
        "mro_ordering",
        "MRO-aware ordering",
        "which declared precedence order should apply",
    )
    NOMINAL_IDENTITY = (
        "nominal_identity",
        "semantic family identity",
        "which semantic role a class or object has",
    )
    PROVENANCE = (
        "provenance",
        "provenance observability",
        "which declaration supplied a fact",
    )
    SHARED_ALGORITHM_AUTHORITY = (
        "shared_algorithm_authority",
        "shared algorithm authority",
        "which algorithm skeleton is authoritative",
    )
    SHARED_TYPE_NAMESPACE = (
        "shared_type_namespace",
        "shared type-namespace authority",
        "which class namespace owns shared behavior",
    )
    TYPE_LINEAGE = (
        "type_lineage",
        "generated-type lineage",
        "which generated type descends from which base identity",
    )
    UNIT_RATE_COHERENCE = (
        "unit_rate_coherence",
        "unit-rate coherence",
        "which fact owner should be authoritative",
    )
    VIRTUAL_MEMBERSHIP = (
        "virtual_membership",
        "explicit virtual membership",
        "which classes explicitly claim a runtime role",
    )
