...
from ..semantic_match import AstTypedEffectStep, GuardedEffectStep, Maybe, RegisteredEffectStep, attribute_call_match, constant_value, named_call_assignment, registered_effect_steps
from ._base import *
from ._helpers import *
from ._substrate_support import *
def _witness_mixin_enforcement_candidate(module: ParsedModule) -> WitnessMixinEnforcementCandidate | None:
    ...
    for candidate in all_classes:
        for token in candidate.family_tokens: grouped[token].append(candidate)
    ...
    for candidate in classes:
        ...
        for role_name, field_names in candidate.normalized_role_fields:
            if role_name not in _WITNESS_MIXIN_ROLE_NAMES: continue
    ...
    if not role_field_names: return None
...
def _shared_field_type_map(observations: tuple[FieldObservation, ...], field_names: tuple[str, ...]) -> tuple[tuple[str, str], ...] | None:
    ...
    for field_name in field_names:
        ...
        if len({fingerprint for fingerprint, _ in annotations}) > 1: return None
        if annotations:
            _, annotation_text = next(iter(annotations))
            if annotation_text is not None: typed_fields.append((field_name, annotation_text))
...
def _field_family_candidates(module: ParsedModule) -> tuple[FieldFamilyCandidate, ...]:
    ...
    for execution_level in (StructuralExecutionLevel.CLASS_BODY, StructuralExecutionLevel.INIT_BODY):
        ...
        for cohort in graph.coherence_cohorts_for(ObservationKind.FIELD, execution_level, minimum_witnesses=2, minimum_fibers=2):
            ...
            if any((len(shared_field_set) / max(len(grouped_by_level[class_name]), 1) < 0.5 for class_name in supporting_classes)): continue
            if any((not grouped_by_level[class_name] - shared_field_set for class_name in supporting_classes)): continue
            ...
            if field_type_map is None: continue
    ...
    for candidate in sorted(candidates, key=lambda item: (item.execution_level, len(item.class_names), len(item.field_names)), reverse=True):
        if any((candidate.execution_level == other.execution_level and set(candidate.class_names) == set(other.class_names) and (set(candidate.field_names) < set(other.field_names)) for other in maximal_candidates)): continue
...
def _role_member_name(tokens: tuple[str, ...]) -> str: return '_'.join(tokens)
def _is_numeric_role_member_name(name: str) -> bool: return all((token.isdigit() for token in name.split('_')))
def _prefixed_role_field_groups(observations: tuple[FieldObservation, ...], *, prefix_token_count: int) -> dict[str, dict[str, FieldObservation]]:
    groups: dict[str, dict[str, FieldObservation]] = defaultdict(dict)
    for observation in observations:
        tokens = _ordered_class_name_tokens(observation.field_name)
        if len(tokens) <= prefix_token_count: continue
        role_name = _role_member_name(tokens[:prefix_token_count])
        member_name = _role_member_name(tokens[prefix_token_count:])
        if not role_name or not member_name: continue
        groups[role_name].setdefault(member_name, observation)
    return groups
def _class_pytree_base_names(node: ast.ClassDef) -> tuple[str, ...]: return tuple((base_name for base_name in _declared_base_names(node) if 'pytree' in base_name.lower()))
def _class_manual_transport_methods(node: ast.ClassDef) -> tuple[str, ...]: return sorted_tuple(_method_names(node) & _PYTREE_TRANSPORT_METHOD_NAMES)
def _connected_role_components(role_to_members: dict[str, dict[str, FieldObservation]], *, min_shared_members: int) -> tuple[tuple[str, ...], ...]:
    ...
    for left_index, left_role in enumerate(roles):
        ...
        for right_role in roles[left_index + 1 :]:
            ...
            if len(shared_members) < min_shared_members: continue
            adjacency[left_role].add(right_role)
            adjacency[right_role].add(left_role)
    components: list[tuple[str, ...]] = []
    seen: set[str] = set()
    for role in roles:
        if role in seen or not adjacency[role]: continue
        stack = [role]
...
def _prefixed_role_bundle_candidate_for_class(module: ParsedModule, class_node: ast.ClassDef, observations: tuple[FieldObservation, ...], config: DetectorConfig) -> PrefixedRoleFieldBundleCandidate | None:
    if len(observations) < config.min_prefixed_role_shared_fields * 2: return None
    ...
    if not (is_dataclass_family or manual_transport_methods or pytree_base_names): return None
    ...
    for prefix_token_count in (1, 2):
        ...
        for role_names in _connected_role_components(role_to_members, min_shared_members=config.min_prefixed_role_shared_fields):
            ...
            if len(shared_member_names) < config.min_prefixed_role_shared_fields: continue
            if all((_is_numeric_role_member_name(member_name) for member_name in shared_member_names)): continue
            if len(shared_member_names) < config.min_prefixed_role_bundle_fields and (not (manual_transport_methods or pytree_base_names)): continue
...
def _prefixed_role_field_bundle_candidates(module: ParsedModule, config: DetectorConfig) -> tuple[PrefixedRoleFieldBundleCandidate, ...]:
    observations: tuple[FieldObservation, ...] = _collect_typed_family_items(module, FieldObservationFamily, FieldObservation)
    observations_by_class: dict[str, list[FieldObservation]] = defaultdict(list)
    for observation in observations:
        if not observation.execution_level.allows_prefixed_role_field_bundle: continue
...
def _prefixed_role_bundle_scaffold(candidate: PrefixedRoleFieldBundleCandidate) -> str:
...
def _public_class_name(name: str) -> str: return ''.join((token.capitalize() for token in _ordered_class_name_tokens(name)))
def _shared_field_base_name(class_names: tuple[str, ...]) -> str:
    suffix = _longest_common_suffix(class_names)
    if suffix: return suffix if suffix.endswith('Base') else f'{suffix}Base'
    prefix = _longest_common_prefix(class_names)
    if prefix: return prefix if prefix.endswith('Base') else f'{prefix}Base'
...
class RepeatedFieldFamilyDetector(CandidateFindingDetector[FieldFamilyCandidate]):
    ...
    def _candidate_items(self, module: ParsedModule, config: DetectorConfig) -> Sequence[object]: del config; return tuple((candidate for candidate in _field_family_candidates(module) if len(candidate.class_names) >= 2 and len(candidate.field_names) >= 2))
    def _finding_for_candidate(self, field_candidate: FieldFamilyCandidate) -> RefactorFinding: evidence = tuple((SourceLocation(item.file_path, item.lineno, item.symbol) for item in field_candidate.observations[:8])); return self.build_finding(f"Classes {', '.join(field_candidate.class_names)} repeat fields {field_candidate.field_names} at `{field_candidate.execution_level}`.", evidence, relation_context=f'same field family repeats across sibling classes at `{field_candidate.execution_level}`', scaffold=_field_family_scaffold(field_candidate), metrics=FieldFamilyMetrics(class_count=len(field_candidate.class_names), field_count=len(field_candidate.field_names), class_names=field_candidate.class_names, field_names=field_candidate.field_names, execution_level=field_candidate.execution_level, dataclass_count=field_candidate.dataclass_count))
...
class ExcessiveBlankLineRunCandidate:
    ...
    @property
    def evidence(self) -> SourceLocation: return SourceLocation(self.file_path, self.start_line, f'blank-lines:{self.blank_line_count}')
...
class ModuleConstructorPolicyFamilyCandidate:
    ...
    @property
    def evidence(self) -> tuple[SourceLocation, ...]: return tuple((SourceLocation(self.file_path, line, row_name) for row_name, line in zip(self.row_names, self.line_numbers, strict=True)))
@dataclass(frozen=True)
class _ConstructorVariantMethod:
    ...
    @property
    def shape_key(self) -> tuple[object, ...]: return (self.callee_name, self.positional_count, self.keyword_names)
@dataclass(frozen=True)
class _AccumulatorFoldMethod:
    ...
    @property
    def shape_key(self) -> tuple[str, str]: return (self.accumulator_type_name, self.result_method_name)
def _simple_classmethod_return_call(node: ast.FunctionDef) -> ast.Call | None:
    if not _is_classmethod(node): return None
...
def _classmethod_constructor_callee_name(call: ast.Call) -> str | None:
    if isinstance(call.func, ast.Name) and call.func.id == 'cls': return 'cls'
    if isinstance(call.func, ast.Attribute) and isinstance(call.func.value, ast.Name) and (call.func.value.id == 'cls'): return f'cls.{call.func.attr}'
    return None
...
def _constructor_variant_method(method: ast.FunctionDef) -> _ConstructorVariantMethod | None:
    ...
    if callee_name is None: return None
    coordinate_fingerprints = _call_coordinate_fingerprints(call)
    if coordinate_fingerprints is None: return None
...
def _varying_coordinate_names(methods: tuple[_ConstructorVariantMethod, ...]) -> tuple[str, ...]:
...
def _class_method_shape_groups(module: ParsedModule, method_parser: Callable[[ast.FunctionDef], _ParsedFamilyMethod | None], shape_key: Callable[[_ParsedFamilyMethod], object]) -> tuple[tuple[ast.ClassDef, tuple[_ParsedFamilyMethod, ...]], ...]:
    ...
    for class_node in (
        ...
        for statement in class_node.body:
            ...
            if method is not None: grouped[shape_key(method)].append(method)
        for methods in grouped.values():
            if len(methods) < 2: continue
            groups.append((class_node, sorted_tuple(methods, key=lambda item: (item.line, item.method_name))))
    return tuple(groups)
def _constructor_variant_family_candidates(module: ParsedModule) -> tuple[ConstructorVariantFamilyCandidate, ...]:
    ...
    for class_node, ordered in _class_method_shape_groups(module, _constructor_variant_method, lambda method: method.shape_key):
        ...
        if not varying_coordinates: continue
...
def _accumulator_fold_method(method: ast.FunctionDef) -> _AccumulatorFoldMethod | None:
    body = _trim_docstring_body(method.body)
    fold_shape = _accumulator_fold_shape(body)
    if fold_shape is None: return None
    ...
    if len(args) <= offset: return None
    source_parameter = args[offset].arg
    if not (isinstance(loop.iter, ast.Name) and loop.iter.id == source_parameter): return None
...
def _accumulator_fold_shape(body: list[ast.stmt]) -> tuple[str, str, ast.For, ast.Call, ast.Call] | None:
    if len(body) != 3: return None
    assign, loop, returned = body
    accumulator = _accumulator_initializer(assign)
    if accumulator is None or not isinstance(loop, ast.For) or (not isinstance(returned, ast.Return)): return None
    ...
    if step_call is None or result_call is None: return None
...
def _accumulator_initializer(statement: ast.stmt) -> tuple[str, str] | None:
    if not isinstance(statement, ast.Assign): return None
    target = as_ast(single_assign_target(statement), ast.Name)
    call = as_ast(statement.value, ast.Call)
    if target is None or call is None or call.args or call.keywords: return None
...
def _accumulator_step_call(loop: ast.For, accumulator_name: str) -> ast.Call | None:
    ...
    if not (target is not None and call is not None and isinstance(call.func, ast.Attribute) and isinstance(call.func.value, ast.Name) and (call.func.value.id == accumulator_name) and (not call.keywords) and isinstance(arg, ast.Name) and (arg.id == target.id)): return None
    return call
def _accumulator_result_call(returned: ast.Return, accumulator_name: str) -> ast.Call | None:
    call = as_ast(returned.value, ast.Call)
    if not (call is not None and isinstance(call.func, ast.Attribute) and isinstance(call.func.value, ast.Name) and (call.func.value.id == accumulator_name) and (not call.args) and (not call.keywords)): return None
    return call
def _accumulator_fold_family_candidates(module: ParsedModule) -> tuple[AccumulatorFoldFamilyCandidate, ...]:
    ...
    for class_node, ordered in _class_method_shape_groups(module, _accumulator_fold_method, lambda method: method.shape_key):
        if len({method.step_method_name for method in ordered}) < 2: continue
...
def _docstring_line_ranges(root: ast.AST) -> set[int]:
    ...
    for node in _walk_nodes(root):
        ...
        if not body or not _is_docstring_expr(body[0]): continue
        start = getattr(body[0], "lineno", None)
        end = getattr(body[0], "end_lineno", start)
        if start is None or end is None: continue
        protected.update(range(start, end + 1))
    return protected
def _excessive_blank_line_run_candidates(module: ParsedModule) -> tuple[ExcessiveBlankLineRunCandidate, ...]:
    protected_lines = _docstring_line_ranges(module.module) | _string_token_line_ranges(module.source)
    candidates: list[ExcessiveBlankLineRunCandidate] = []
    run_start: int | None = None
    run_length = 0
    def flush(end_line: int) -> None:
        nonlocal run_start, run_length
        if run_start is not None and run_length: candidates.append(ExcessiveBlankLineRunCandidate(file_path=str(module.path), start_line=run_start, end_line=end_line, blank_line_count=run_length))
        run_start = None
        run_length = 0
    for line_number, line in enumerate(module.source.splitlines(), 1):
        if line_number in protected_lines or line.strip():
            flush(line_number - 1)
            continue
        if run_start is None: run_start = line_number
        run_length += 1
    flush(line_number if "line_number" in locals() else 0)
    return tuple(candidates)
def _string_token_line_ranges(source: str) -> set[int]:
    protected: set[int] = set()
    for token in tokenize.generate_tokens(io.StringIO(source).readline):
        if token.type == tokenize.STRING and token.start[0] != token.end[0]: protected.update(range(token.start[0], token.end[0] + 1))
    return protected
def _catalog_installing_mixin_candidate(method: ast.FunctionDef) -> str | None: return cast(str | None, Maybe.of(method).bind_all(registered_effect_steps(_CatalogInstallingMixinStep)).unwrap_or_none())
...
class _NamedFunctionExprCallPairStep(AstTypedEffectStep[ast.FunctionDef, _FunctionCallPairResult], Generic[_FunctionCallPairResult]):
    ...
    def project_ast(self, value: ast.FunctionDef) -> _FunctionCallPairResult | None:
        if value.name != self.function_name: return None
        ...
        if first_call is None or second_call is None: return None
        return self.project_call_pair(first_call, second_call)
    @abstractmethod
    def project_call_pair(self, first_call: ast.Call, second_call: ast.Call) -> _FunctionCallPairResult | None: raise NotImplementedError
class _CatalogInitSubclassBodyStep(_CatalogInstallingMixinStep, _NamedFunctionExprCallPairStep[_CatalogInstallingMixinShape]):
    step_id = "catalog_init_subclass_body"
    registration_order = 10
    function_name = "__init_subclass__"
    def project_call_pair(self, first_call: ast.Call, second_call: ast.Call) -> _CatalogInstallingMixinShape: return _CatalogInstallingMixinShape(first_call, second_call)
class _CatalogSuperInitSubclassStep(_CatalogInstallingMixinStep, GuardedEffectStep[_CatalogInstallingMixinShape, _CatalogInstallingMixinShape]):
    step_id = "catalog_super_init_subclass"
    registration_order = 20
    def project(self, value: _CatalogInstallingMixinShape) -> _CatalogInstallingMixinShape | None:
        ...
        if match is None or name_id(match.owner.func) != 'super': return None
        return value
class _CatalogInstallAttributeStep(_CatalogInstallingMixinStep, GuardedEffectStep[_CatalogInstallingMixinShape, str]):
...
def _catalog_installing_mixin_family_candidates(module: ParsedModule) -> tuple[CatalogInstallingMixinFamilyCandidate, ...]:
    ...
    for class_node in (
        ...
        for statement in class_node.body:
            ...
            if catalog_attribute is not None: items.append((class_node.name, catalog_attribute, statement.lineno))
...
class _RegexGroupExtractorStep(RegisteredEffectStep):
    pass
class _RegexExtractorBodyStep(_RegexGroupExtractorStep, AstTypedEffectStep[ast.FunctionDef, _RegexExtractorBody]):
...
class _RegexExtractorAssignmentStep(_RegexGroupExtractorStep, GuardedEffectStep[_RegexExtractorBody, _RegexExtractorAssignment]):
...
class _RegexExtractorMatcherCallStep(_RegexGroupExtractorStep, GuardedEffectStep[_RegexExtractorAssignment, _RegexExtractorMatcherCall]):
    step_id = "regex_extractor_matcher_call"
    registration_order = 30
    def project(self, value: _RegexExtractorAssignment) -> _RegexExtractorMatcherCall | None:
...
def _regex_conditional_group_call(value: _RegexExtractorMatcherCall) -> ast.Call | None:
    ...
    if ifexp is None or name_id(ifexp.test) != value.match_name or none_orelse is None or (none_orelse.value is not None) or (group_call is None): return None
    return group_call
class _RegexExtractorConditionalReturnStep(_RegexGroupExtractorStep, GuardedEffectStep[_RegexExtractorMatcherCall, _RegexExtractorConditionalReturn]):
    step_id = "regex_extractor_conditional_return"
    registration_order = 40
    def project(self, value: _RegexExtractorMatcherCall) -> _RegexExtractorConditionalReturn | None:
        group_call = _regex_conditional_group_call(value)
        if group_call is None: return None
...
class _RegexExtractorGroupCallStep(_RegexGroupExtractorStep, GuardedEffectStep[_RegexExtractorConditionalReturn, _RegexGroupExtractorMethod]):
    step_id = "regex_extractor_group_call"
    registration_order = 50
    def project(self, value: _RegexExtractorConditionalReturn) -> _RegexGroupExtractorMethod | None:
        ...
        if not isinstance(group_index, int): return None
...
def _regex_group_extractor_method(method: ast.FunctionDef) -> _RegexGroupExtractorMethod | None: return cast(_RegexGroupExtractorMethod | None, Maybe.of(method).bind_all(registered_effect_steps(_RegexGroupExtractorStep)).unwrap_or_none())
def _regex_group_extractor_family_candidates(module: ParsedModule) -> tuple[RegexGroupExtractorFamilyCandidate, ...]:
    ...
    for class_node in (
        ...
        for method in methods: grouped[method.group_index].append(method)
        for group_index, grouped_methods in grouped.items():
            if len(grouped_methods) < 2: continue
    ...
    return tuple(candidates)
def _class_has_constructor_variant_mixin(node: ast.ClassDef) -> bool: return any((_ast_terminal_name(base) == 'ConstructorVariantMixin' for base in node.bases))
def _sparse_constructor_variant_family_candidates(module: ParsedModule) -> tuple[SparseConstructorVariantFamilyCandidate, ...]:
    candidates: list[SparseConstructorVariantFamilyCandidate] = []
    for class_node in (
        node for node in _walk_nodes(module.module) if isinstance(node, ast.ClassDef)
    ):
        if not _is_dataclass_class(class_node) or _class_has_constructor_variant_mixin(class_node): continue
        field_names = set(_dataclass_field_names(class_node))
        methods: list[tuple[ast.FunctionDef, tuple[str, ...]]] = []
        for statement in class_node.body:
            if not isinstance(statement, ast.FunctionDef) or not _is_classmethod(statement): continue
...
def _support_prelude_import_name(module_node: ast.Module) -> str | None:
    for statement in module_node.body:
        if not isinstance(statement, ast.ImportFrom): continue
        if len(statement.names) != 1 or statement.names[0].name != '*': continue
        imported_module = statement.module or ""
        if 'support' not in imported_module.lower(): continue
        return "." * statement.level + imported_module
    return None
def _module_has_family_catalog(module_path: Path) -> bool:
    if not module_path.exists(): return False
    try:
    ...
    for node in tree.body:
        ...
        if not any((isinstance(target, ast.Name) and 'MODULE' in target.id and ('CATALOG' in target.id or 'MANIFEST' in target.id) for target in node.targets)): continue
        if isinstance(node.value, (ast.Tuple, ast.List, ast.Set, ast.Call)): return True
    return False
...
def _support_prelude_module_family_candidates(modules: list[ParsedModule]) -> tuple[SupportPreludeModuleFamilyCandidate, ...]:
    ...
    for module in modules:
        ...
        if len(top_level_classes) != 1 or top_level_functions: continue
        support_import = _support_prelude_import_name(module.module)
        if support_import is None: continue
        support_path = _support_module_path(module, support_import)
        if support_path is not None and _module_has_family_catalog(support_path): continue
    ...
    return tuple(candidates)
def _is_module_policy_row_name(name: str) -> bool: return name.isupper() and '_' in name
def _constructor_call_schema(call: ast.Call) -> tuple[str, ...]: return (*(f'arg{index}' for index, _arg in enumerate(call.args)), *(keyword.arg or '**' for keyword in call.keywords))
def _module_constructor_policy_family_candidates(module: ParsedModule) -> tuple[ModuleConstructorPolicyFamilyCandidate, ...]:
    ...
    for row_name, (line, call) in _module_level_named_calls(module).items():
        if not _is_module_policy_row_name(row_name): continue
        schema = _constructor_call_schema(call)
        if len(schema) < 2: continue
...
declare_module_detector(ConstructorVariantFamilyCandidate, high_confidence_certified_spec(PatternId.AUTHORITATIVE_SCHEMA, 'Constructor variants should derive from one constructor algebra', 'Several classmethods on one class are pure constructor vectors over the same target. The varying coordinates are data, not independent algorithms, so the family should be derived from one typed variant catalog.', 'single constructor-variant catalog that derives named class constructors', 'same class has sibling classmethods that return the same constructor target with a shared coordinate schema', _AUTHORITATIVE_NOMINAL_IDENTITY_SHARED_ALGORITHM_AUTHORITY_CAPABILITY_TAGS, _CLASS_FAMILY_NORMALIZED_AST_MANUAL_SYNCHRONIZATION_OBSERVATION_TAGS), CandidateFindingRenderer[ConstructorVariantFamilyCandidate](summary=lambda variant_candidate: f'`{variant_candidate.class_name}` repeats constructor target `{variant_candidate.callee_name}` across methods {variant_candidate.method_names}; varying coordinates are {variant_candidate.varying_coordinate_names}.', evidence=lambda variant_candidate: variant_candidate.evidence, scaffold=lambda variant_candidate: '@dataclass(frozen=True)\nclass ConstructorVariantSpec:\n    name: str\n    args: tuple[ConstructorArg, ...]\n\nclass ConstructorVariantMixin:\n    __constructor_variants__: ClassVar[ConstructorVariantCatalog]\n    def __init_subclass__(cls):\n        cls.__constructor_variants__.install(cls)', codemod_patch=lambda variant_candidate: f'# Replace classmethods {variant_candidate.method_names} on `{variant_candidate.class_name}` with one typed constructor-variant catalog.\n# Each method name becomes data; one mixin derives the bound classmethods from the catalog.', metrics=lambda variant_candidate: MappingMetrics(mapping_site_count=len(variant_candidate.method_names), field_count=variant_candidate.coordinate_count, mapping_name=variant_candidate.class_name, field_names=variant_candidate.varying_coordinate_names)), candidate_collector=_constructor_variant_family_candidates)
declare_module_detector(AccumulatorFoldFamilyCandidate, high_confidence_certified_spec(PatternId.ABC_TEMPLATE_METHOD, 'Accumulator folds should derive from one fold algebra', 'Several methods instantiate the same accumulator, stream one source iterable through different accumulator step hooks, and return the same projection. The loop skeleton is an algebraic fold and should be one reusable composition primitive.', 'single accumulator-fold substrate with declarative step hooks', 'same owner class repeats accumulator initialization, loop, and result projection with only the step hook varying', _SHARED_ALGORITHM_AUTHORITY_AUTHORITATIVE_NOMINAL_IDENTITY_CAPABILITY_TAGS, _NORMALIZED_AST_MANUAL_SYNCHRONIZATION_OBSERVATION_TAGS), CandidateFindingRenderer[AccumulatorFoldFamilyCandidate](summary=lambda fold_candidate: f'`{fold_candidate.class_name}` repeats `{fold_candidate.accumulator_type_name}` folds across methods {fold_candidate.method_names}; step hooks are {fold_candidate.step_method_names} and result hook is `{fold_candidate.result_method_name}`.', evidence=lambda fold_candidate: fold_candidate.evidence, scaffold=lambda fold_candidate: '@dataclass(frozen=True)\nclass AccumulatorFoldSpec:\n    name: str\n    step_method_name: str\n\nclass AccumulatorFoldMixin:\n    __accumulator_folds__: ClassVar[AccumulatorFoldCatalog]\n    def __init_subclass__(cls):\n        cls.__accumulator_folds__.install(cls)', codemod_patch=lambda fold_candidate: f'# Replace fold methods {fold_candidate.method_names} on `{fold_candidate.class_name}` with one accumulator-fold catalog.\n# Keep accumulator type and result projection in one authority; each source method only declares its step hook.', metrics=lambda fold_candidate: RepeatedMethodMetrics.from_duplicate_family(duplicate_site_count=len(fold_candidate.method_names), statement_count=3, class_count=1, method_symbols=tuple((f'{fold_candidate.class_name}.{name}' for name in fold_candidate.method_names)))), candidate_collector=_accumulator_fold_family_candidates)
declare_module_detector(ExcessiveBlankLineRunCandidate, high_confidence_certified_spec(PatternId.LOCAL_VALUE_AUTHORITY, 'Nonsemantic blank source regions should be collapsed', 'A contiguous run of blank source lines outside docstrings carries no semantic information. It inflates the module and hides true structure without adding an abstraction boundary.', 'compact source layout with no nonsemantic blank-line payload', 'source contains an empty region larger than a canonical separator', _SHARED_ALGORITHM_AUTHORITY_AUTHORITATIVE_CAPABILITY_TAGS, (ObservationTag.NORMALIZED_AST,)), CandidateFindingRenderer[ExcessiveBlankLineRunCandidate](summary=lambda blank_candidate: f'`{blank_candidate.file_path}` has {blank_candidate.blank_line_count} contiguous blank lines from {blank_candidate.start_line} to {blank_candidate.end_line}.', evidence=lambda blank_candidate: (blank_candidate.evidence,), scaffold=lambda blank_candidate: '# Delete the nonsemantic blank-line run.\n# Keep at most the canonical separator needed by the surrounding declarations.', codemod_patch=lambda blank_candidate: f'# Collapse blank lines {blank_candidate.start_line}-{blank_candidate.end_line} in `{blank_candidate.file_path}`.', metrics=lambda blank_candidate: RepeatedMethodMetrics.from_duplicate_family(duplicate_site_count=blank_candidate.blank_line_count, statement_count=1, class_count=0, method_symbols=('blank-line-run',))), candidate_collector=_excessive_blank_line_run_candidates)
declare_module_detector(MultilineStringLiteralCandidate, high_confidence_certified_spec(PatternId.LOCAL_VALUE_AUTHORITY, 'Split string literal should collapse into one literal authority', 'A non-docstring string constant spread across several physical source lines has one runtime value. The extra line structure is not semantic; it should collapse into one literal authority.', 'single source literal for one runtime string value', 'one string constant is split across multiple physical source lines', _AUTHORITATIVE_SHARED_ALGORITHM_AUTHORITY_CAPABILITY_TAGS, _NORMALIZED_AST_OBSERVATION_TAGS), CandidateFindingRenderer[MultilineStringLiteralCandidate](summary=lambda literal: f'`{literal.file_path}` has a {literal.line_count}-line string literal at {literal.line}-{literal.end_line}.', evidence=lambda literal: (literal.evidence,), scaffold=lambda literal: "literal = 'single source string with escaped newlines when needed'", codemod_patch=lambda literal: f'# Replace the physical string split at {literal.file_path}:{literal.line}-{literal.end_line} with one escaped literal.', metrics=lambda literal: MappingMetrics.from_field_names(mapping_site_count=literal.line_count, mapping_name='multiline-string-literal', field_names=('runtime_string_value',))), candidate_collector=_multiline_string_literal_candidates)
declare_module_detector(MultilineFStringLiteralCandidate, high_confidence_certified_spec(PatternId.LOCAL_VALUE_AUTHORITY, 'Split formatted string literal should collapse into one literal authority', 'A formatted string assembled from several adjacent physical string tokens still has one runtime value and one interpolation graph. The token split is not semantic; keep the formatted value in one literal authority.', 'single source literal for one runtime formatted string value', 'one formatted string value is split across multiple physical source tokens', _AUTHORITATIVE_SHARED_ALGORITHM_AUTHORITY_CAPABILITY_TAGS, _NORMALIZED_AST_OBSERVATION_TAGS), CandidateFindingRenderer[MultilineFStringLiteralCandidate](summary=lambda literal: f'`{literal.file_path}` has a {literal.line_count}-line formatted string literal at {literal.line}-{literal.end_line} with {literal.expression_count} interpolations.', evidence=lambda literal: (literal.evidence,), scaffold=lambda literal: 'literal = f"single source string with {interpolation} and escaped newlines"', codemod_patch=lambda literal: f'# Replace the physical formatted-string split at {literal.file_path}:{literal.line}-{literal.end_line} with one escaped f-string literal.', metrics=lambda literal: MappingMetrics.from_field_names(mapping_site_count=literal.line_count, mapping_name='multiline-f-string-literal', field_names=('runtime_formatted_string_value',))), candidate_collector=_multiline_f_string_literal_candidates)
...
declare_module_detector(StraightlineSimpleFunctionCandidate, high_confidence_certified_spec(PatternId.LOCAL_VALUE_AUTHORITY, 'Straightline simple function suite should collapse into one function authority', 'A comment-free function body containing only simple statements has no nested control structure. The physical suite split is layout, not behavior; the function authority can keep the same simple-statement sequence in one suite.', 'single source function suite for one straightline statement sequence', 'one function body is split across multiple physical lines while containing only simple statements', _AUTHORITATIVE_SHARED_ALGORITHM_AUTHORITY_CAPABILITY_TAGS, _NORMALIZED_AST_OBSERVATION_TAGS), CandidateFindingRenderer[StraightlineSimpleFunctionCandidate](summary=lambda function_candidate: f'`{function_candidate.file_path}` has a {function_candidate.line_count}-line straightline `{function_candidate.function_name}` function with {function_candidate.statement_count} simple statement(s).', evidence=lambda function_candidate: (function_candidate.evidence,), scaffold=lambda function_candidate: 'def function(...): statement_a; statement_b; return value', codemod_patch=lambda function_candidate: f'# Replace the comment-free straightline function suite at {function_candidate.file_path}:{function_candidate.line}-{function_candidate.end_line} with one simple-statement suite.', metrics=lambda function_candidate: MappingMetrics.from_field_names(mapping_site_count=function_candidate.line_count, mapping_name=function_candidate.function_name, field_names=function_candidate.statement_kinds)), candidate_collector=_straightline_simple_function_candidates)
declare_module_detector(SingleStatementControlSuiteCandidate, high_confidence_certified_spec(PatternId.LOCAL_VALUE_AUTHORITY, 'Single-statement control suite should collapse into one control authority', 'A comment-free control statement whose body is exactly one simple statement has one guard/iteration authority and one action. The extra physical suite split is not semantic.', 'single source control suite for one guarded simple statement', 'one control suite is split across multiple physical lines despite containing exactly one simple statement', _AUTHORITATIVE_SHARED_ALGORITHM_AUTHORITY_CAPABILITY_TAGS, _NORMALIZED_AST_OBSERVATION_TAGS), CandidateFindingRenderer[SingleStatementControlSuiteCandidate](summary=lambda suite_candidate: f'`{suite_candidate.file_path}` has a {suite_candidate.line_count}-line {suite_candidate.control_kind} suite at {suite_candidate.line}-{suite_candidate.end_line} with one {suite_candidate.statement_kind} statement.', evidence=lambda suite_candidate: (suite_candidate.evidence,), scaffold=lambda suite_candidate: 'if condition: return value', codemod_patch=lambda suite_candidate: f'# Replace the comment-free single-statement {suite_candidate.control_kind} suite at {suite_candidate.file_path}:{suite_candidate.line}-{suite_candidate.end_line} with one simple-statement suite.', metrics=lambda suite_candidate: MappingMetrics.from_field_names(mapping_site_count=suite_candidate.line_count, mapping_name=suite_candidate.control_kind, field_names=(suite_candidate.statement_kind,))), candidate_collector=_single_statement_control_suite_candidates)
declare_module_detector(MultilineDefinitionHeaderCandidate, high_confidence_certified_spec(PatternId.LOCAL_VALUE_AUTHORITY, 'Split definition header should collapse into one definition authority', 'A comment-free multi-line function or class header is one definition boundary. The split signature layout is not semantic; the header should be represented by one source authority.', 'single source definition header for one function or class boundary', 'one function or class definition header is split across multiple physical source lines without comments', _AUTHORITATIVE_SHARED_ALGORITHM_AUTHORITY_CAPABILITY_TAGS, _NORMALIZED_AST_OBSERVATION_TAGS), CandidateFindingRenderer[MultilineDefinitionHeaderCandidate](summary=lambda header_candidate: f'`{header_candidate.file_path}` has a {header_candidate.line_count}-line {header_candidate.definition_kind} header for `{header_candidate.definition_name}` at {header_candidate.line}-{header_candidate.end_line}.', evidence=lambda header_candidate: (header_candidate.evidence,), scaffold=lambda header_candidate: 'def function(arg_a, arg_b) -> Result:', codemod_patch=lambda header_candidate: f'# Replace the comment-free split definition header at {header_candidate.file_path}:{header_candidate.line}-{header_candidate.end_line} with one header line.', metrics=lambda header_candidate: MappingMetrics.from_field_names(mapping_site_count=header_candidate.line_count, mapping_name=header_candidate.definition_name, field_names=(header_candidate.definition_kind,))), candidate_collector=_multiline_definition_header_candidates)
...
declare_module_detector(RegexGroupExtractorFamilyCandidate, high_confidence_certified_spec(PatternId.AUTHORITATIVE_SCHEMA, 'Regex group extractor methods should derive from descriptors', 'Several methods repeat `match = pattern.<mode>(text); return match.group(n) if match else None`. The pattern field and matcher mode are data; the extractor algorithm should be one descriptor or helper substrate.', 'one regex group extraction descriptor with declared pattern and matcher coordinates', 'same class repeats regex group extractor methods over different pattern fields', _AUTHORITATIVE_SHARED_ALGORITHM_AUTHORITY_NOMINAL_IDENTITY_CAPABILITY_TAGS, _NORMALIZED_AST_MANUAL_SYNCHRONIZATION_OBSERVATION_TAGS), CandidateFindingRenderer[RegexGroupExtractorFamilyCandidate](summary=lambda regex_candidate: f'`{regex_candidate.class_name}` repeats regex group-{regex_candidate.group_index} extractors {regex_candidate.method_names} over patterns {regex_candidate.pattern_attribute_names}.', evidence=lambda regex_candidate: regex_candidate.evidence, scaffold=lambda regex_candidate: "@dataclass(frozen=True)\nclass RegexGroupExtractor:\n    pattern_attr: str\n    matcher_name: str = 'search'\n    group_index: int = 1\n    def __get__(self, instance, owner): ...", codemod_patch=lambda regex_candidate: '# Replace repeated regex extractor methods with descriptor rows.\n# Each method name becomes a descriptor assignment declaring pattern attribute, matcher mode, and group index.', metrics=lambda regex_candidate: MappingMetrics.from_field_names(mapping_site_count=len(regex_candidate.method_names), mapping_name=regex_candidate.class_name, field_names=regex_candidate.pattern_attribute_names)), candidate_collector=_regex_group_extractor_family_candidates)
declare_module_detector(SparseConstructorVariantFamilyCandidate, high_confidence_certified_spec(PatternId.AUTHORITATIVE_SCHEMA, 'Sparse dataclass constructor variants should derive from one variant catalog', 'Several classmethods on one dataclass construct the same record while overriding sparse subsets of defaulted fields. Those sparse overrides are rows in the constructor algebra, not independent methods.', 'single sparse constructor-variant catalog over dataclass defaults', 'same dataclass repeats classmethod constructors that override different keyword subsets', _AUTHORITATIVE_NOMINAL_IDENTITY_SHARED_ALGORITHM_AUTHORITY_CAPABILITY_TAGS, _CLASS_FAMILY_KEYWORD_MANUAL_SYNCHRONIZATION_OBSERVATION_TAGS), CandidateFindingRenderer[SparseConstructorVariantFamilyCandidate](summary=lambda sparse_candidate: f'`{sparse_candidate.class_name}` repeats sparse constructor variants {sparse_candidate.method_names} over defaulted fields {sparse_candidate.keyword_names}.', evidence=lambda sparse_candidate: sparse_candidate.evidence, scaffold=lambda sparse_candidate: "ConstructorVariantCatalog(\n    (ConstructorVariantSpec(name='...', parameters=(), args=(), kwargs=(...)),)\n)", codemod_patch=lambda sparse_candidate: f"# Replace sparse classmethods {sparse_candidate.method_names} on `{sparse_candidate.class_name}` with constructor-variant rows.\n# Keep dataclass defaults as the base point and declare only each variant's overridden fields.", metrics=lambda sparse_candidate: sparse_candidate.mapping_metrics), candidate_collector=_sparse_constructor_variant_family_candidates)
class SupportPreludeModuleFamilyDetector(IssueDetector):
    ...
    def _collect_findings(self, modules: list[ParsedModule], config: DetectorConfig) -> list[RefactorFinding]:
        ...
        return findings
declare_module_detector(ModuleConstructorPolicyFamilyCandidate, high_confidence_certified_spec(PatternId.AUTHORITATIVE_SCHEMA, 'Module constructor policy rows should derive from a semantic catalog', 'Several module-level constant rows instantiate the same policy constructor with the same argument schema. Those rows are semantic data, so the architecture should derive them from one role/catalog authority rather than spell each constructor call by hand.', 'one constructor-row catalog keyed by semantic policy role', 'same module has multiple constant rows assigned from the same constructor shape', _AUTHORITATIVE_NOMINAL_IDENTITY_UNIT_RATE_COHERENCE_CAPABILITY_TAGS, _KEYWORD_NORMALIZED_AST_PARTIAL_VIEW_OBSERVATION_TAGS), CandidateFindingRenderer[ModuleConstructorPolicyFamilyCandidate](summary=lambda policy_candidate: f"Module constants {', '.join(policy_candidate.row_names)} repeat `{policy_candidate.constructor_name}` constructor rows with schema {policy_candidate.field_names}.", evidence=lambda policy_candidate: policy_candidate.evidence, scaffold=lambda policy_candidate: '@dataclass(frozen=True)\nclass PolicyRowSpec:\n    role_name: str\n    constructor_args: tuple[object, ...]\n\nclass PolicyCatalog:\n    def materialize(self) -> dict[str, object]: ...', codemod_patch=lambda policy_candidate: '# Replace repeated module-level constructor rows with one semantic policy catalog.\n# Keep role names and constructor coordinates as data, then derive the module constants from the catalog.', metrics=lambda policy_candidate: MappingMetrics(mapping_site_count=len(policy_candidate.row_names), field_count=len(policy_candidate.field_names), mapping_name=policy_candidate.constructor_name, field_names=policy_candidate.row_names)), candidate_collector=_module_constructor_policy_family_candidates)
declare_module_detector(DynamicSelfFieldSelectionCandidate, high_confidence_spec(PatternId.CONFIG_CONTRACTS, 'Dynamic self-field selection hides a nominal contract', 'A class selects one of its own fields through reflective indirection instead of declaring one fail-loud hook or one canonical field.', 'declared nominal count/value hook instead of selector-driven reflective lookup', 'class template selects its own state through dynamic reflective field names', _FAIL_LOUD_CONTRACTS_NOMINAL_IDENTITY_PROVENANCE_CAPABILITY_TAGS), CandidateFindingRenderer[DynamicSelfFieldSelectionCandidate](summary=lambda dynamic_candidate: f'`{dynamic_candidate.class_name}.{dynamic_candidate.method_name}` uses `{dynamic_candidate.reflective_builtin}(self, {dynamic_candidate.selector_expression})` instead of one declared nominal hook.', evidence=lambda dynamic_candidate: (dynamic_candidate.evidence,), scaffold=lambda dynamic_candidate: 'class DeclaredCountHook(ABC):\n    @property\n    @abstractmethod\n    def count_value(self) -> int: ...', codemod_patch=lambda dynamic_candidate: f'# Delete `{dynamic_candidate.reflective_builtin}(self, {dynamic_candidate.selector_expression})`.\n# Replace selector-driven reflection with one declared property or one canonical field on the nominal carrier.'), candidate_collector=_dynamic_self_field_selection_candidates)
declare_module_detector(StringBackedReflectiveNominalLookupCandidate, high_confidence_spec(PatternId.NOMINAL_BOUNDARY, 'String-backed reflective lookup is simulating nominal identity', 'The docs say a class family should not smuggle behavior through string selectors and reflection. When subclasses only supply constant names that are resolved through globals, getattr, or __dict__, the boundary should become one declared nominal hook or typed handle.', 'declared nominal hook or typed family handle instead of string selector plus reflection', 'class family encodes behavior with constant selector strings and resolves it reflectively', _NOMINAL_IDENTITY_FAIL_LOUD_CONTRACTS_PROVENANCE_CAPABILITY_TAGS, _STRING_DISPATCH_SEMANTIC_STRING_LITERAL_CLASS_FAMILY_OBSERVATION_TAGS), CandidateFindingRenderer[StringBackedReflectiveNominalLookupCandidate](summary=lambda reflective_candidate: f'`{reflective_candidate.class_name}.{reflective_candidate.method_name}` resolves `{reflective_candidate.selector_attr_name}` through `{reflective_candidate.lookup_kind}` over {len(reflective_candidate.concrete_class_names)} concrete classes.', evidence=lambda reflective_candidate: (reflective_candidate.evidence,), scaffold=lambda reflective_candidate: 'class DeclaredNominalRole(ABC):\n    @classmethod\n    @abstractmethod\n    def declared_handle(cls) -> object: ...', codemod_patch=lambda reflective_candidate: f'# Delete the reflective `{reflective_candidate.lookup_kind}` lookup keyed by `{reflective_candidate.selector_attr_name}`.\n# Move the family boundary to one declared hook, typed handle, or polymorphic method.', metrics=lambda reflective_candidate: SentinelSimulationMetrics(class_count=len(reflective_candidate.concrete_class_names), branch_site_count=1)), detector_base=ConfiguredModuleCollectorCandidateDetector, candidate_collector=_string_backed_reflective_nominal_lookup_candidates)
